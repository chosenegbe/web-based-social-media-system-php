    ...
    <table>
        <tr>
            <td><form action="accountSetting.php" method="POST" id='customPassword'>
                ...
                <?php $Query_User = mysql_query("SELECT * FROM users WHERE username = '$user'");
                      while($rowPass = mysql_fetch_assoc($Query_User)){
                      $pass = $rowPass['password'];
                      $id = $rowPass['id'];
                      ?>
                        <a rel="tooltip"  title="Edit" id="e<?php echo $id; ?>" href="#edit<?php echo $id; ?>" 
                data-toggle="modal" class="btn btn-success"><i class="icon-pencil icon-large">Edit password
                </i></a>
               	    <?php include('updateAccount/modal_edit_password.php'); ?>
                <?php
